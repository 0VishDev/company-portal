...
class SettingsController extends Controller
{
    ...
    public function payment_settings()
    {
        ...
        $payment_option = array('paypal', 'stripe', '2checkout', 'authorize.net', 'paystack', 'localbank');
    ...
    public function update_payment_settings(Request $request)
    {
        ...
        $site_product_inquiry_fee = (strlen($request->input('site_product_inquiry_fee')) > 0 ? $request->input('site_product_inquiry_fee') : 0);
    ...
    {
        $isCountryDeleted = Country::where('country_id', $countryId)->delete();
        
        $states = State::where('country_id', $countryId)->get();
        
        $isCityDeleted = City::where('state_id', $states->pluck('id')->toArray())->delete();
        
        $isStateDeleted = State::where('country_id', $countryId)->delete();
    ...
    public function state_settings()
    {
        ...
        $state['data'] = Settings::getstateData();
        return view('admin.state-settings', ['state' => $state]);
    }
    ...
    public function save_state(Request $request)
    {
        ...
        $rules = array(
            'state_name' => ['required', 'max:255', Rule::unique('state')->where(function ($sql) {$sql->where('state_name', '!=', '');})],
        );
        ...
        if ($validator->fails()) {
            ...
            if ($request->hasFile('state_image')) {
                ...
                $img_name = time() . '.' . $image->getClientOriginalExtension();
                $destinationPath = public_path('/storage/states');
                $imagePath = $destinationPath . "/" . $img_name;
                $image->move($destinationPath, $img_name);
                $path = '/storage/states/' . $img_name;
            } else {
                $path = $request->input('save_state_image');
            }

            $data = array('country_id' => $user_country, 'state_name' => $state_name, 'state_image' => $path, 'show_in_home' => $show_in_home);

            Settings::savestateData($data);
            return redirect('/admin/state-settings')->with('success', 'Insert successfully.');
    ...
    {
        $isStateDeleted = State::where('id', $stateId)->delete();
        
        $isCityDeleted = City::where('state_id', $stateId)->delete();

        return redirect()->back()->with('success', 'Delete successfully.');
    }

    public function edit_state($cid)
    {

        $edit['state'] = Settings::editState($cid);
        return view('admin.edit-state', ['edit' => $edit, 'cid' => $cid]);
    }
    ...
    public function update_state(Request $request)
    {
        ...
        $rules = array(
            'state_name' => ['required', 'max:255', Rule::unique('state')->ignore($cid, 'id')->where(function ($sql) {$sql->where('state_name', '!=', '');})],
        ...
        if ($validator->fails()) {
            ...
            if ($request->hasFile('state_image')) {
                ...
                File::delete('public/' . $request->input('save_state_image'));
            ...
            Settings::updatestateData($cid, $data);
            return redirect('/admin/state-settings')->with('success', 'Update successfully.');
    ...
    public function city_settings()
    {
        ...
        $city['data'] = Settings::getcityData();
        
        return view('admin.city-settings', ['city' => $city]);
    }
    ...
    public function save_city(Request $request)
    {
        ...
        $rules = array(
            ...
            'city_name' => ['required', 'max:255', Rule::unique('city')->where(function ($sql) {$sql->where('city_name', '!=', '');})],
        );
        ...
        if ($validator->fails()) {
            ...
            $data = array('country_id' => $user_country, 'state_id' => $user_state, 'city_name' => $city_name);

            Settings::savecityData($data);
            return redirect('/admin/city-settings')->with('success', 'Insert successfully.');
    ...
    public function delete_city($cid)
    {
        ...
        Settings::deleteCitydata($cid);

        return redirect()->back()->with('success', 'Delete successfully.');

    }

    public function edit_city($cid)
    {

        $edit['city'] = Settings::editCity($cid);
        return view('admin.edit-city', ['edit' => $edit, 'cid' => $cid]);
    }
    ...
    public function update_city(Request $request)
    {
        ...
        $rules = array(
            'city_name' => ['required', 'max:255', Rule::unique('city')->ignore($cid, 'id')->where(function ($sql) {$sql->where('city_name', '!=', '');})],
        ...
        if ($validator->fails()) {
            ...
            Settings::updatecityData($cid, $data);
            return redirect('/admin/city-settings')->with('success', 'Update successfully.');
