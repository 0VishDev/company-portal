...
class PagesController extends Controller
{
    ...
    {
        $visitors =  Visitors::where('user_id', '<>', 0)->orderBy('visit_time', 'desc')->get()->unique('user_id');
        
        foreach($visitors as $key => $visitor){
            $visitors[$key]->total_visit_count = Visitors::where('user_id', $visitor->user_id)->sum('visit_count');
        }
