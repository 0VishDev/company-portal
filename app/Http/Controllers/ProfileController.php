...
class ProfileController extends Controller
{
    ...
    public function buyer_profile()
    {
        ...
        $productInquiry = ProductInquiry::where('user_id', $user->id)->orWhere('email', $user->email)->get();
    
        return view('buyer', compact('user', 'productInquiry'));
    }
    ...
    public function view_myprofile()
    {
        ...
        if(strlen($edit['profile']->country_id) > 0){
            $states = State::where('country_id','=', $edit['profile']->country_id)->get();
            $cities = City::where('state_id','=', $edit['profile']->state_id)->get();
        }else{
            $states = State::where('country_id','=', $countries[0]->country_id)->get();
            $cities = City::where('state_id','=', (isset($states[0]) ? $states[0]->id : ''))->get();
        }
        
        return view('my-profile', compact('edit', 'countries', 'states', 'cities'));
    ...
    public function update_myprofile(Request $request)
    {
		...
		// Custom Fields
		$address_short = $request->input('address_short');
		...
		$legal_status_of_firm = $request->input('legal_status_of_firm');
        ...
        if ($validator->fails()) {
			...
			if ($request->hasFile('brochure')) {
				...
				Members::droBrochure($token);

                $brochure = $request->file('brochure');
                $brochure_name = time() . '45.' . $brochure->getClientOriginalExtension();
                $destinationPath = public_path('/storage/users');
                $brochurePath = $destinationPath . "/" . $brochure_name;
                $brochure->move($destinationPath, $brochure_name);
                $user_brochure = $img_name;
            } else {
                $user_brochure = $request->input('save_brochure');
			}

            $data = array('name' => $name, 'username' => $username, 'email' => $email, 'password' => $pass, 'company_name' => $company_name, 'country_id' => $user_country, 'state_id' => $user_state, 'city_id' => $user_city, 'user_gender' => $user_gender,
                'user_photo' => $user_image, 'user_banner' => $user_banner, 'user_address' => $user_address, 'user_phone' => $user_phone, 'user_about' => $user_about,
                'updated_at' => date('Y-m-d H:i:s'), 'address_short' => $address_short, 'gst_no' => $gst_no, 'nature_of_business' => $nature_of_business, 'no_of_employees' => $no_of_employees, 'year_of_establishment' => $year_of_establishment, 'legal_status_of_firm' => $legal_status_of_firm, 'annual_turnover' => $annual_turnover, 'certification' => $certification, 'facebook' => $facebook, 'linkedin' => $linkedin, 'twitter' => $twitter, 'brochure_path' => $brochure);
