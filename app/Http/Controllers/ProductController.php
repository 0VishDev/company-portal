...
class ProductController extends Controller
{
    ...
    public function show_wishlist()
    {
        ...
        $setting['setting'] = Settings::editGeneral($sid);
        $user_id = Auth::user()->id;
        $shop['product'] = Product::with('ProductImages')->join('wishlist', 'wishlist.product_token', 'product.product_token')->where('product.product_status', '=', 1)->where('product.product_drop_status', '=', 'no')->where('wishlist.user_id', '=', $user_id)->orderBy('product_id', 'desc')->get();
    ...
    public function confirm_paystack(Request $request)
    {
        ...
        $purchase_token = $request->input('purchase_token');
        $amount = base64_decode($request->input('amount')) * 100;
    ...
    public function show_cart()
    {
        ...
        $user_id = Auth::user()->id;
        $cart['product'] = Product::viewOrder($user_id);
        $data = array('setting' => $setting, 'cart' => $cart);
    ...
    public function view_cart(Request $request)
    {
        ...
        $savedata = array('user_id' => $user_id, 'product_id' => $product_id, 'product_user_id' => $product_user_id, 'product_token' => $product_token, 'token' => $token, 'quantity' => $product_quantity, 'product_attribute' => $product_attribute, 'product_attribute_values' => $product_attribute_values, 'price' => $product_price, 'order_status' => $order_status);
        $updata = array('quantity' => $product_quantity, 'product_attribute' => $product_attribute, 'product_attribute_values' => $product_attribute_values);
        if ($product_stock >= $product_quantity && $product_quantity > 0) {
            ...
            if ($check_order == 0) {
                ...
                Product::updateOrder($user_id, $order_status, $product_token, $updata);
    ...
    public function edit_product($product_token)
    {
        ...
        $attributer['display'] = Attribute::with('AttributeValue')->where('attribute_status', '=', '1')->where('attribute_drop_status', '=', 'no')->orderBy('attribute_order', 'asc')->get();
        return view('edit-product', ['edit' => $edit, 'product_categories' => $product_categories, 'editimage' => $editimage, 'product_attribute' => $product_attribute, 'attributer' => $attributer, 'brand' => $brand]);
    }
    ...
    public function update_product(Request $request)
    {
		...
		$minimum_order_unit = $request->input('minimum_order_unit');
    ...
    public function add_product()
    {
        ...
        $attributer['display'] = Attribute::with('AttributeValue')->where('attribute_status', '=', '1')->where('attribute_drop_status', '=', 'no')->orderBy('attribute_order', 'asc')->get();
        $brand['view'] = Product::homebrandData();
        return view('add-product', ['attributer' => $attributer, 'brand' => $brand]);
    }
