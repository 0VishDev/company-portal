             ...
             {{ csrf_field() }}
                ...
                <div class="form-group row">
                    <div class="col-sm-6">
                        ...
                        <select class="form-control" name="user_country" id="country" data-bvalidator="required">
                        @foreach($countries as $country)
                        <option value="{{ $country->country_id }}" @if($edit['profile']->country_id == $country->country_id) selected @endif>{{ $country->country_name }}</option>
                        @endforeach
                        </select>
                    </div>
                    <div class="col-sm-6">
                        <label for="state">State <span class="required">*</span></label>
                        <select class="form-control" name="user_state" id="state" data-bvalidator="required">
                        @foreach($states as $state)
                        <option value="{{ $state->id }}" @if($edit['profile']->state_id == $state->id) selected @endif>{{ $state->state_name }}</option>
                        @endforeach
                        </select>
                    </div>
                    <div class="col-sm-6">
                        <label for="city">City <span class="required">*</span></label>
                        <select class="form-control" name="user_city" id="city" data-bvalidator="required">
                        @foreach($cities as $city)
                        <option value="{{ $city->id }}" @if($edit['profile']->city_id == $city->id) selected @endif>{{ $city->city_name }}</option>
                        @endforeach
                        </select>
                    </div>
                    <div class="col-sm-6">
                ...
                <div class="form-group row">
                    <div class="col-sm-6 mb-2">
                        ...
                        <input type="text" class="form-control" id="company_name" name="company_name" value="{{ $edit['profile']->company_name }}">
                    </div>
                    <div class="col-sm-6 mb-2">
                ...
                <input type="hidden" name="save_brochure" value="{{ $edit['profile']->user_banner }}">
                <input type="hidden" name="user_brochure" value="{{ $edit['profile']->brochure_path }}">
